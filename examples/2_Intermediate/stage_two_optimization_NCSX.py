...
for j in range(N_INITIAL_GUESS_PERTURBATIONS):
    ...
    def fun(dofs):
        ...
        outstr += f", C-C-Sep={Jccdist.shortest_distance():.2f}, #C-S-Sep={Jccdist.shortest_distance():.2f}"
...
header_string = 'Jf.J(), <Perturbed Jf.J()>'
combined_array = np.column_stack((np.array(sq_flux_values),np.array(mean_perturbed_sq_flux_values)))
